...
class PrintBridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            ezpl_base64_list = data.get("ezpl_base64_list")
            ezpl_list = data.get("ezpl_list")

            if ezpl_base64_list is not None:
                if not isinstance(ezpl_base64_list, list) or not ezpl_base64_list:
                    raise ValueError(
                        "ezpl_base64_list must be a non-empty list"
                    )
                raw_items = []
                for index, b64 in enumerate(ezpl_base64_list, start=1):
                    if not isinstance(b64, str) or not b64.strip():
                        raise ValueError(
                            f"ezpl_base64_list item {index} is empty"
                        )
                    raw_items.append(base64.b64decode(b64))

            elif ezpl_list is not None:
                ...
                for index, ezpl in enumerate(ezpl_list, start=1):
                    ...
                    raw_items.append(ezpl.encode("latin-1"))

            else:
                raise ValueError(
                    "Request must include 'ezpl_base64_list' or 'ezpl_list'"
                )

            job_ids = []
            for index, raw_data in enumerate(raw_items, start=1):
                job_id = raw_print(
                    raw_data,
