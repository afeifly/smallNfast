...
function openAddModal() {
  modalMode.value = 'add';
  productForm.value = { item: '', name: '', band: 'atlascopco' };
...
function openEditModal(product) {
  ...
  productForm.value = { item: product.item, name: product.name, band: product.band || 'atlascopco' };
  productFormError.value = '';
  showProductModal.value = true;
}
...
@media (max-width: 768px) {
  .label-maker { padding: 16px; }
  h1 { font-size: 2rem; }
  form, .labels-preview, .products-management { padding: 24px; }
  .label-card { width: 320px; height: 200px; }
